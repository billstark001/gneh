...
export function defineIRFragment(
  ...
  const containsSuspend = (body: StoryNode[]): boolean =>
    body.some((node) => {
      if (node.type === 'suspend') return true;
      if (node.type === 'if') return containsSuspend(node.yes) || containsSuspend(node.no);
      return 'children' in node && containsSuspend(node.children);
    });
  ...
  const resumeCondition = (
    node: Extract<StoryNode, { type: 'suspend' }>,
    ctx: FragmentContext,
    scope: Scope,
  ): ResumeCondition => {
    switch (node.resume.type) {
      case 'manual':
        return { type: 'manual' };
      case 'timer':
        return {
          type: 'timer',
          durationMs: Number(evaluateValue(node.resume.durationMs, ctx, scope)),
          clock: node.resume.clock,
        };
      case 'signal':
        return {
          type: 'signal',
          name: node.resume.name,
          filter: node.resume.filter
            ? (evaluateValue(node.resume.filter, ctx, scope) as import('@gneh/core').Json)
            : undefined,
        };
      case 'task':
        return {
          type: 'task',
          operation: node.resume.operation,
          args: node.resume.args.map((argument) => evaluateValue(argument, ctx, scope) as import('@gneh/core').Json),
        };
    }
  };
  ...
  function renderNodes(body: StoryNode[], ctx: FragmentContext, parent: Scope, path: string, existing = false): View[] {
    ...
    for (let index = 0; index < body.length; index++) {
      ...
      const viewKey = `${ctx.instanceId}${key.slice(ctx.mountKey.length)}`;
      const rendered = ((): View[] => {
        switch (node.type) {
          case 'text':
            return [{ kind: 'text', text: node.value, key: viewKey }];
          case 'content':
            return [
              {
                kind: node.kind,
          ...
          case 'effect':
            ...
            return [];
          case 'suspend': {
            const suspensionKey = `${path}/suspend:${node.id ?? index}`;
            const active = ctx.suspend(
              suspensionKey,
              resumeCondition(node, ctx, scope),
              node.bind
                ? (value, actionContext) => bindPattern(node.bind!, value ?? null, actionContext, scope)
                : undefined,
            );
            return [{ kind: 'suspend', key: suspensionKey, attrs: { active } }];
          }
          case 'value':
            return [{ kind: 'text', key: viewKey, text: display(valueAt(node.expression, ctx, scope, key)) }];
          case 'if': {
            const continuationKey = `flow:branch:${key}`;
            const resumable = containsSuspend(node.yes) || containsSuspend(node.no);
            const selected = resumable
              ? ctx.continuation(continuationKey, (initial) =>
                  Boolean(valueAt(node.test, initial, scope, key + '/test')),
                )
              : Boolean(valueAt(node.test, ctx, scope, key + '/test'));
            const branch = nodes(selected ? node.yes : node.no, ctx, scope, key + '/branch');
            if (resumable && !ctx.suspended) ctx.deleteContinuation(continuationKey);
            return branch;
          }
          case 'each': {
            const resumable = containsSuspend(node.children);
            const items = resumable
              ? ctx.continuation(`flow:items:${key}`, (initial) => {
                  const initialItems = valueAt(node.items, initial, scope, key + '/items');
                  invariant(Array.isArray(initialItems), 'E_ITERABLE', 'A structural loop requires an array.');
                  return initialItems as import('@gneh/core').Json[];
                })
              : valueAt(node.items, ctx, scope, key + '/items');
            ...
            for (let i = 0; i < items.length; i++) {
              ...
              const identity = node.key
                ...
                : item && typeof item === 'object' && Object.hasOwn(item, 'id')
                  ...
                  : i;
              invariant(
                ['string', 'number'].includes(typeof identity),
                'E_KEY',
                'Loop keys must be strings or numbers.',
              );
              ...
              childScope._index = i;
              childScope.index = i;
              children.push(...nodes(node.children, ctx, childScope, key + '/' + stable, true));
              if (ctx.suspended) break;
            }
            if (resumable && !ctx.suspended) ctx.deleteContinuation(`flow:items:${key}`);
            return children;
          }
          case 'include': {
          ...
          case 'call': {
            ...
            if (isAuthoredCallable(target)) {
              invariant(
                target.phase === 'view',
                'E_CALLABLE_PHASE',
                `Cannot call a ${target.phase} callable as a view.`,
              );
              return ctx.evaluate(target.invoke(...args, ctx) as import('@gneh/core').RenderInput, `${key}/authored`)
                .view;
            }
            if (target.phase === 'value')
              return [{ kind: 'text', key: viewKey, text: display(runtime.invokeValue(target, args, ctx)) }];
  ...
  return definePassage({
    ...
    render: (ctx: FragmentContext, props: FragmentProps) => {
      ...
      return nodes(ir.body, ctx, root, ctx.mountKey, true);
    },
  });
}
