...
A file without a Twee header is one passage. Its ID comes from front matter `id` or from the filename. A multi-passage file may have file front matter before the first header and passage front matter immediately after each header.
...
Precedence is file front matter, then header JSON, then passage front matter. Objects merge recursively, arrays replace earlier values, and `tags` is a stable union. The example keeps the display name `Card` while using the runtime ID `EnemyCard`.
